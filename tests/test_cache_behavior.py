...
from ai_utilities.cache import MemoryCache, SqliteCache, stable_hash
...
class TestCacheBehavior:
    ...
    def test_cache_key_different_inputs(self, fake_settings):
        ...
        key2 = client._build_cache_key("ask", prompt="different prompt", **{
        ...
        key4 = client._build_cache_key("ask", **{
            **base_params,
        ...
        key6 = client._build_cache_key("ask", **{**base_params, "return_format": "json"})
        assert key5 != key6
    ...
    def test_cache_with_different_parameters(self, fake_settings):
        ...
        provider = FakeProvider(responses=["Response for {prompt} with temp {temperature}"])
        client = AiClient(settings=fake_settings, provider=provider)
        ...
        response2 = client.ask("test", temperature=0.8)  # Above cache threshold but still different
        assert provider.call_count == 2
        
        # Same prompt, same parameters should use cache
        response3 = client.ask("test", temperature=0.5)

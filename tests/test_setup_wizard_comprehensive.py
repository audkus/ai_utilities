...
from ai_utilities.setup.wizard import (
    SetupMode, SetupResult, SetupWizard, run_setup_wizard
)
...
class TestSetupWizardBasic:
    ...
    def test_run_setup_wizard_convenience_function(self):
        ...
        with patch.object(SetupWizard, 'run_wizard', return_value=mock_result) as mock_run:
...
class TestSetupWizardModes:
    ...
    def test_normal_mode_configuration(self):
        ...
        with patch.dict(os.environ, {**clean_env, "AI_API_KEY": "test-key"}, clear=True):
    ...
    def test_enhanced_mode_configuration(self):
        ...
        with patch.dict(os.environ, {**clean_env, "OPENAI_API_KEY": "enhanced-key"}, clear=True):
            result = wizard.run_wizard(
    ...
    def test_improved_mode_configuration(self):
        ...
        with patch.dict(os.environ, {**clean_env, "AI_API_KEY": "improved-key"}, clear=True):
            result = wizard.run_wizard(
...
class TestSetupWizardProviders:
    ...
    def test_provider_configuration_structure(self):
        ...
        expected_providers = ['openai', 'groq', 'together', 'openrouter', 'openai_compatible', 'ollama']
        
        for provider_id, config in wizard.providers.items():
            assert provider_id in expected_providers, f"Unexpected provider: {provider_id}"
            ...
            assert config['default_model'] is None or isinstance(config['default_model'], str)
    ...
    def test_provider_api_key_priority(self):
        ...
        test_cases = [
            ...
            ({"AI_API_KEY": "ai-key", "OPENAI_API_KEY": "openai-key"}, "openai-key"),  # OPENAI_API_KEY takes priority
        ]
...
class TestSetupWizardFileOperations:
    ...
    def test_dotenv_file_update_preserves_existing(self):
        ...
        try:
            ...
            with patch.dict(os.environ, {**clean_env, "AI_API_KEY": "new-key"}, clear=True):
                wizard.run_wizard(
                    mode=SetupMode.NORMAL,
                    dotenv_path=str(dotenv_path),
                    dry_run=False,
...
class TestSetupWizardEnvironmentDetection:
    ...
    def test_environment_variable_detection(self):
        ...
        test_env = {
            ...
            "AI_BASE_URL": "https://custom.url"
        }
        
        with patch.dict(os.environ, {**clean_env, **test_env}, clear=True):
            result = wizard.run_wizard(
                mode=SetupMode.NORMAL,
                dotenv_path=".env",
                dry_run=True,

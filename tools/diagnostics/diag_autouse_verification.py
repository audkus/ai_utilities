...
def test_autouse_fixture_clears_environment():
    ...
    assert os.environ.get("AI_PROVIDER") == "openai"  # noqa: S101 - Test validation
    assert os.environ.get("AI_API_KEY") == "test-key-from-test"  # noqa: S101 - Test validation
    assert os.environ.get("OPENAI_API_KEY") == "openai-key-from-test"  # noqa: S101 - Test validation
...
def test_autouse_fixture_isolation():
    ...
    assert ai_provider is None, f"Expected AI_PROVIDER to be None, got {ai_provider}"  # noqa: S101 - Test validation
    assert ai_api_key is None, f"Expected AI_API_KEY to be None, got {ai_api_key}"  # noqa: S101 - Test validation
    assert openai_api_key is None, (  # noqa: S101 - Test validation
        f"Expected OPENAI_API_KEY to be None, got {openai_api_key}"
    )

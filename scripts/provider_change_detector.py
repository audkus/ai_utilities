...
class ProviderChangeDetector:
    ...
    def _generate_report(
        self, results: Dict[str, ProviderStatus], analysis: Dict[str, Any]
    ) -> str:
        ...
        report.append(
            f"- **Overall Health**: {healthy}/{total} providers healthy ({healthy / total * 100:.1f}%)"
        )
        report.append(f"- **Critical Issues**: {len(analysis['critical_issues'])}")
        report.append(f"- **Warnings**: {len(analysis['warnings'])}")
        report.append(
            f"- **Performance Issues**: {len(analysis['performance_issues'])}"
        )
        report.append("")
        ...
        report.append(
            "- Consider implementing fallback providers for degraded services"
        )
    ...
    def run_tests_with_monitoring(self):
        ...
        failed_providers = [
            name for name, status in health_results.items() if status.status == "down"
        ]

        if failed_providers:
            print(
                f"⚠️  Skipping tests for failed providers: {', '.join(failed_providers)}"
            )
...
"""
    ...
    # Write script to current directory for CI/CD use
    # In production, consider using a dedicated output directory
    script_path = "ci_provider_check.sh"
    with open(script_path, "w") as f:
        f.write(script)

    os.chmod(script_path, 0o755)
    print(f"📄 Created CI/CD script: {script_path}")
